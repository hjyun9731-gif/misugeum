...
root = Path(".")
route_file = root / "report_export_routes.py"
...
if "report_export_routes" not in main_text:
    append_code = '''
...
'''
    main_text += append_code
    main_file.write_text(main_text, encoding="utf-8")
    print("DONE: main.py에 report export routes 연결 완료")
else:
    print("SKIP: main.py에 이미 report export routes가 연결되어 있음")
...
if "openpyxl" not in req.lower():
    ...
    print("DONE: requirements.txt에 openpyxl 추가 완료")
else:
    print("SKIP: openpyxl 이미 있음")

print("DONE: N월 부과대수 / N월 입금추출 엑셀 기능 파일 생성 완료")
