...
def classify_bank_income_types():
    try:
        import re
        from models import BankTransaction, Member
        ...
        for m in members:
            ...
            l4 = last4(veh)
            if l4 and len(l4) >= 3:
                member_last4s.add(l4)

        def looks_like_member_payment(memo):
            cm = compact(memo)
            dm = digits(memo)

            for nm in member_names:
                if len(nm) >= 2 and nm in cm:
                    return True

            for l4 in member_last4s:
                if len(l4) >= 3 and l4 in dm:
                    return True
        ...
        return {
            "status": "ok",
            "checked": checked,
            "to_misc_income": to_misc,
            "to_suspense": to_suspense,
            "skipped": skipped,
            "examples_misc_income": examples_misc,
            "examples_suspense": examples_suspense,
        }
